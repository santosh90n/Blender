...
GreasePencilModifierData *ED_object_gpencil_modifier_add(ReportList *reports, Main *bmain, Scene *scene, Object *ob, const char *name, int type)
{
	GreasePencilModifierData *md = NULL, *new_md = NULL;
	...
	if (mti->flags & eGreasePencilModifierTypeFlag_Single) {
...
bool ED_object_gpencil_modifier_remove(ReportList *reports, Main *bmain, Object *ob, GreasePencilModifierData *md)
{
...
void ED_object_gpencil_modifier_clear(Main *bmain, Object *ob)
{
	...
	while (md) {
		GreasePencilModifierData *next_md;
...
int ED_object_gpencil_modifier_move_up(ReportList *reports, Object *ob, GreasePencilModifierData *md)
{
	if (md->prev) {
		BLI_remlink(&ob->greasepencil_modifiers, md);
...
int ED_object_gpencil_modifier_move_down(ReportList *reports, Object *ob, GreasePencilModifierData *md)
{
	if (md->next) {
		BLI_remlink(&ob->greasepencil_modifiers, md);
...
int ED_object_gpencil_modifier_copy(ReportList *UNUSED(reports), Object *ob, GreasePencilModifierData *md)
{
	GreasePencilModifierData *nmd;

	nmd = BKE_gpencil_modifier_new(md->type);
	modifier_copyData(md, nmd);
	BLI_insertlinkafter(&ob->greasepencil_modifiers, md, nmd);
	modifier_unique_name(&ob->greasepencil_modifiers, nmd);
        ...
        bContext *C, PointerRNA *UNUSED(ptr), PropertyRNA *UNUSED(prop), bool *r_free)
{
	...
	const GreasePencilModifierTypeInfo *mti;
	int totitem = 0, a;
	...
	for (a = 0; rna_enum_object_modifier_type_items[a].identifier; a++) {
		...
		if (md_item->identifier[0]) {
			...
			if (mti->flags & eGreasePencilModifierTypeFlag_NoUserAdd)
				continue;

			if ((mti->flags & eGreasePencilModifierTypeFlag_GpencilMod) == 0) {
				continue;
			}
		}
		else {
...
int edit_modifier_poll_generic(bContext *C, StructRNA *rna_type, int obtype_flag)
{
	...
	if (ID_IS_STATIC_OVERRIDE(ob)) {
		...
		return (((GreasePencilModifierData *)ptr.data)->flag & eModifierFlag_StaticOverride_Local) != 0;
	}
...
GreasePencilModifierData *edit_modifier_property_get(wmOperator *op, Object *ob, int type)
{
...
static int modifier_remove_exec(bContext *C, wmOperator *op)
{
	...
	Object *ob = ED_object_active_context(C);
	GreasePencilModifierData *md = edit_modifier_property_get(op, ob, 0);
